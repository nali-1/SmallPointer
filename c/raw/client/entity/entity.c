#ifdef SMPTR_MDxM0
	static SMPTRtMA Pma_pomi_tea0[] =
	{
		SMPTR_MDeM_POMI_TEA,
		SMPTR_MDeM_POMI_MF1,
		SMPTR_MDeM_POMI_MM1,
		SMPTR_MDeM_POMI_M,
		SMPTR_MDeM_POMI_TEA_HAT,
		SMPTR_MDeM_POMI_TEA_BACK
	};
	static const struct SMPTR_CE_ETTsM Sm_pomi_tea0 =
	{
		.Uj = SMPTR_MDePOMI,
		.Lma = sizeof(Pma_pomi_tea0) / sizeof(Pma_pomi_tea0[0]),
		.Pma = Pma_pomi_tea0
	};

	static SMPTRtMA Pma_pomi_tea1[] =
	{
		SMPTR_MDeM_POMI_TEA,
		SMPTR_MDeM_POMI_MF1,
		SMPTR_MDeM_POMI_MM0,
		SMPTR_MDeM_POMI_M,
		SMPTR_MDeM_POMI_TEA_HAT,
		SMPTR_MDeM_POMI_TEA_BACK
	};
	static const struct SMPTR_CE_ETTsM Sm_pomi_tea1 =
	{
		.Uj = SMPTR_MDePOMI,
		.Lma = sizeof(Pma_pomi_tea1) / sizeof(Pma_pomi_tea1[0]),
		.Pma = Pma_pomi_tea1
	};

	static SMPTRtMA Pma_pomi_tea2[] =
	{
		SMPTR_MDeM_POMI_TEA,
		SMPTR_MDeM_POMI_MF0000,
		SMPTR_MDeM_POMI_MF00,
		SMPTR_MDeM_POMI_MF0,
		SMPTR_MDeM_POMI_MM1,
		SMPTR_MDeM_POMI_M,
		SMPTR_MDeM_POMI_TEA_HAT,
		SMPTR_MDeM_POMI_TEA_BACK
	};
	static const struct SMPTR_CE_ETTsM Sm_pomi_tea2 =
	{
		.Uj = SMPTR_MDePOMI,
		.Lma = sizeof(Pma_pomi_tea2) / sizeof(Pma_pomi_tea2[0]),
		.Pma = Pma_pomi_tea2
	};

	static SMPTRtMA Pma_pomi_tea3[] =
	{
		SMPTR_MDeM_POMI_TEA,
		SMPTR_MDeM_POMI_MF0000,
		SMPTR_MDeM_POMI_MF00,
		SMPTR_MDeM_POMI_MF0,
		SMPTR_MDeM_POMI_MM0,
		SMPTR_MDeM_POMI_M,
		SMPTR_MDeM_POMI_TEA_HAT,
		SMPTR_MDeM_POMI_TEA_BACK
	};
	static const struct SMPTR_CE_ETTsM Sm_pomi_tea3 =
	{
		.Uj = SMPTR_MDePOMI,
		.Lma = sizeof(Pma_pomi_tea3) / sizeof(Pma_pomi_tea3[0]),
		.Pma = Pma_pomi_tea3
	};

	static SMPTRtMA Pma_pomi_tea4[] =
	{
		SMPTR_MDeM_POMI_TEA,
		SMPTR_MDeM_POMI_MF01,
		SMPTR_MDeM_POMI_MF0,
		SMPTR_MDeM_POMI_MM1,
		SMPTR_MDeM_POMI_M,
		SMPTR_MDeM_POMI_TEA_HAT,
		SMPTR_MDeM_POMI_TEA_BACK
	};
	static const struct SMPTR_CE_ETTsM Sm_pomi_tea4 =
	{
		.Uj = SMPTR_MDePOMI,
		.Lma = sizeof(Pma_pomi_tea4) / sizeof(Pma_pomi_tea4[0]),
		.Pma = Pma_pomi_tea4
	};

	static SMPTRtMA Pma_pomi_tea5[] =
	{
		SMPTR_MDeM_POMI_TEA,
		SMPTR_MDeM_POMI_MF01,
		SMPTR_MDeM_POMI_MF0,
		SMPTR_MDeM_POMI_MM0,
		SMPTR_MDeM_POMI_M,
		SMPTR_MDeM_POMI_TEA_HAT,
		SMPTR_MDeM_POMI_TEA_BACK
	};
	static const struct SMPTR_CE_ETTsM Sm_pomi_tea5 =
	{
		.Uj = SMPTR_MDePOMI,
		.Lma = sizeof(Pma_pomi_tea5) / sizeof(Pma_pomi_tea5[0]),
		.Pma = Pma_pomi_tea5
	};

	static SMPTRtMA Pma_pomi_2core[] =
	{
		SMPTR_MDeM_POMI_2CORE,
		SMPTR_MDeM_POMI_MF0000,
		SMPTR_MDeM_POMI_MF00,
		SMPTR_MDeM_POMI_MF0,
		SMPTR_MDeM_POMI_MM1,
		SMPTR_MDeM_POMI_M,
		SMPTR_MDeM_POMI_IShovel
	};
	static const struct SMPTR_CE_ETTsM Sm_pomi_2core =
	{
		.Uj = SMPTR_MDePOMI,
		.Lma = sizeof(Pma_pomi_2core) / sizeof(Pma_pomi_2core[0]),
		.Pma = Pma_pomi_2core
	};
#endif

#ifdef SMPTR_MDxM1
	static SMPTRtMA Pma_ui_rain0[] =
	{
		SMPTR_MDeM_UI_RAIN,
		SMPTR_MDeM_UI_001,
		SMPTR_MDeM_UI_M,
		SMPTR_MDeM_UI_IClover,
		SMPTR_MDeM_UI_MF0,
		SMPTR_MDeM_UI_MF01,
		SMPTR_MDeM_UI_MM0
	};
	static const struct SMPTR_CE_ETTsM Sm_ui_rain0 =
	{
		.Uj = SMPTR_MDeUI,
		.Lma = sizeof(Pma_ui_rain0) / sizeof(Pma_ui_rain0[0]),
		.Pma = Pma_ui_rain0
	};

	static SMPTRtMA Pma_ui_rain1[] =
	{
		SMPTR_MDeM_UI_RAIN,
		SMPTR_MDeM_UI_001,
		SMPTR_MDeM_UI_M,
		SMPTR_MDeM_UI_IClover,
		SMPTR_MDeM_UI_MF1,
		SMPTR_MDeM_UI_MM0
	};
	static const struct SMPTR_CE_ETTsM Sm_ui_rain1 =
	{
		.Uj = SMPTR_MDeUI,
		.Lma = sizeof(Pma_ui_rain1) / sizeof(Pma_ui_rain1[0]),
		.Pma = Pma_ui_rain1
	};

	static SMPTRtMA Pma_ui_rain2[] =
	{
		SMPTR_MDeM_UI_RAIN,
		SMPTR_MDeM_UI_001,
		SMPTR_MDeM_UI_M,
		SMPTR_MDeM_UI_IClover,
		SMPTR_MDeM_UI_MF0,
		SMPTR_MDeM_UI_MF02,
		SMPTR_MDeM_UI_MM0
	};
	static const struct SMPTR_CE_ETTsM Sm_ui_rain2 =
	{
		.Uj = SMPTR_MDeUI,
		.Lma = sizeof(Pma_ui_rain2) / sizeof(Pma_ui_rain2[0]),
		.Pma = Pma_ui_rain2
	};

	static SMPTRtMA Pma_croakie_rain0[] =
	{
		SMPTR_MDeM_CROAKIE_RAIN,
		SMPTR_MDeM_CROAKIE_C0
	};
	static const struct SMPTR_CE_ETTsM Sm_croakie_rain0 =
	{
		.Uj = SMPTR_MDeCROAKIE,
		.Lma = sizeof(Pma_croakie_rain0) / sizeof(Pma_croakie_rain0[0]),
		.Pma = Pma_croakie_rain0
	};

	static SMPTRtMA Pma_croakie_rain1[] =
	{
		SMPTR_MDeM_CROAKIE_RAIN,
		SMPTR_MDeM_CROAKIE_C1
	};
	static const struct SMPTR_CE_ETTsM Sm_croakie_rain1 =
	{
		.Uj = SMPTR_MDeCROAKIE,
		.Lma = sizeof(Pma_croakie_rain1) / sizeof(Pma_croakie_rain1[0]),
		.Pma = Pma_croakie_rain1
	};
#endif

const struct SMPTR_CE_ETTsM SMPTR_CE_ETTpM[] =
{
	#ifdef SMPTR_MDxM0
		Sm_pomi_tea0,
		Sm_pomi_tea1,
		Sm_pomi_tea2,
		Sm_pomi_tea3,
		Sm_pomi_tea4,
		Sm_pomi_tea5,
		Sm_pomi_2core,
	#endif
	#ifdef SMPTR_MDxM1
		Sm_ui_rain0,
		Sm_ui_rain1,
		Sm_ui_rain2,
		Sm_croakie_rain0,
		Sm_croakie_rain1,
	#endif
};

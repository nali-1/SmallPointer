...
//	SMPT_DBmN2L("J %d", Pmix[sizeof(float) * 3 + 1])
	...
	{
		if (!memcmp(Ph14t[Uh14][U0].P, Pmix, lM))
		{
	...
	*(Pi[Ui] + Pil[Ui]) = Lh14i;
	...
	Ph14t[Uh14] = realloc(Ph14t[Uh14], lM * Ph14tl[Uh14] + lM);
	memcpy(Ph14t[Uh14][Ph14tl[Uh14]].P, Pmix, lM);
	++Ph14tl[Uh14];

	++Lh14i;
}

static void Mset_mesh(cgltf_data *Pcgltf_data, const char **Pm[], const uint8_t Pml[], uint32_t U0)
{
	uint16_t Ui;

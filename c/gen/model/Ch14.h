#define lH14T (1024*8)

static uint8_t (*Ph14_t[lH14T])[14];
static SMPTRtI *Ph14_ti[lH14T];
static uint32_t Ph14_tl[lH14T] = {0};
static uint32_t Lh14_i = 0;

static SMPTRtI *Ph14_i[SMPTR_MDcM];
static SMPTRtI Ph14_il[SMPTR_MDcM] = {0};

//.i https://en.wikipedia.org/wiki/Fowler%E2%80%93Noll%E2%80%93Vo_hash_function
static uint64_t Mh14(uint8_t *P, uint8_t L)
{
	uint64_t h = 1469598103934665603ULL;
	for (uint8_t U0 = 0; U0 < L; U0++)
	{
		h ^= P[U0];
		//.i FNV prime 64bit
		h *= 1099511628211ULL;
	}
	return h;
}

static void Mh14_mesh(uint8_t *Pmix, uint16_t Ui, uint8_t Lm)
{
	//! hash 14b
//	SMPT_DBmN2L("V0 %f", *(float *)Pmix)
//	SMPT_DBmN2L("V1 %f", *(float *)(Pmix + sizeof(float)))
//	SMPT_DBmN2L("V2 %f", *(float *)(Pmix + sizeof(float) * 2))
//	SMPT_DBmN2L("C %d", Pmix[sizeof(float) * 3])
//	SMPT_DBmN2L("J %d", Pmix[sizeof(float) * 3 + 1])

	uint16_t Uh14 = Mh14(Pmix, Lm) % lH14T;
	for (uint32_t U0 = 0; U0 < Ph14_tl[Uh14]; ++U0)
	{
		if (!memcmp(Ph14_t[Uh14][U0], Pmix, Lm))
		{
			Ph14_i[Ui] = realloc(Ph14_i[Ui], Ph14_il[Ui] * sizeof(SMPTRtI) + sizeof(SMPTRtI));
			*(Ph14_i[Ui] + Ph14_il[Ui]) = Ph14_ti[Uh14][U0];
			++Ph14_il[Ui];
			return;
		}
	}

	Ph14_i[Ui] = realloc(Ph14_i[Ui], Ph14_il[Ui] * sizeof(SMPTRtI) + sizeof(SMPTRtI));
	*(Ph14_i[Ui] + Ph14_il[Ui]) = Lh14_i;
	++Ph14_il[Ui];

	Ph14_ti[Uh14] = realloc(Ph14_ti[Uh14], sizeof(SMPTRtI) * Ph14_tl[Uh14] + sizeof(SMPTRtI));
	Ph14_ti[Uh14][Ph14_tl[Uh14]] = Lh14_i;

	Ph14_t[Uh14] = realloc(Ph14_t[Uh14], Lm * Ph14_tl[Uh14] + Lm);
	memcpy(Ph14_t[Uh14][Ph14_tl[Uh14]], Pmix, Lm);
	++Ph14_tl[Uh14];

	++Lh14_i;
}

static void Mh14_clean()
{
	for (uint8_t U0 = 0; U0 < Lh14_i; U0++)
	{
		Ph14_t[U0] = realloc(Ph14_t[U0], 0);
		Ph14_ti[U0] = realloc(Ph14_ti[U0], 0);
	}
	memset(Ph14_tl, 0, sizeof(uint32_t) * lH14T);
	Lh14_i = 0;
}

static void Mh14_set()
{
	for (uint32_t U0 = 0; U0 < lH14T; ++U0)
	{
		Ph14_t[U0] = malloc(0);
		Ph14_ti[U0] = malloc(0);
	}

	for (SMPTRtMA U0 = 0; U0 < SMPTR_MDcM; ++U0)
		Ph14_i[U0] = malloc(0);
}

static void Mh14_free()
{
	for (SMPTRtMA U0 = 0; U0 < SMPTR_MDcM; ++U0)
		free(Ph14_i[U0]);

	for (uint32_t U0 = 0; U0 < lH14T; ++U0)
	{
		free(Ph14_t[U0]);
		free(Ph14_ti[U0]);
	}
}

		...
		else
		{
			...
			#endif
			{
				//SMPT_DBmN2L("m1")
					...
					++Ldst;
					Pvkwritedescriptorset = realloc(Pvkwritedescriptorset, SMPT_RD_VKW_DSTS_LOlMAIN * smpt_rd_vk_swcUimage * Ldst * sizeof(VkWriteDescriptorSet));
...
//				Pm->Ut[1] = Pm->Sm.Ut;
				...
				Pm->Um = Pm->Sm.Um;
				//! c
				memcpy(Pm->Sm0.Ptr, Pm->Sm.Sm0.Ptr, sizeof(float) * Pm->Sm.Sm0.Ltr);
